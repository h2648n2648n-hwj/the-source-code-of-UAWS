...
class DQNScheduler:
    ...
    use_attention: bool = True  # 新增：是否使用注意力网络
    ):
        ...
        # 初始化网络：根据use_attention切换
        if self.use_attention:
            print("DQN: using AttnNetwork")
            self.dqn_net        = AttnNetwork(state_dim, action_num, device=self.device).to(self.device);
            self.dqn_target_net = AttnNetwork(state_dim, action_num, device=self.device).to(self.device);
        else:
    ...
    def schedule1(self, last_part, task, vm_list, ready_queue, remained_task, all_task_num, now_time, done):
        ...
        valid_cnt = len(vm_list)
        valid_mask = [1] * valid_cnt + [0] * (self.action_num - valid_cnt)
        action, q = self.selectAction(state, now_time, task.id,
                                      valid_actions=valid_cnt)
        ...
        if self.dqn_net.training:
            ...
            if done:
                ...
                if self.rewards_episode:
                    ep_mean = float(sum(self.rewards_episode) / len(self.rewards_episode))
                    self.episode_mean_rewards.append(ep_mean)
                    self.rewards_episode.clear()
                self.transition = [];
                self.last_time = 0;
                self.last_task = None;
    ...
    def train(self):
        ...
        if len(self.memory) >= self.batch_size:
            ...
            self.global_step += 1
            self.epsilon = self.epsilon_end + (self.epsilon_start - self.epsilon_end) * math.exp(-1.0 * self.global_step * self.epsilon_decay)
    ...
    def selectAction(self, state, now_time=0, tid=0,
                     valid_actions=None, last_part=True):
        ...
        if valid_actions is None:
            valid_actions = self.action_num
        def mask_invalid(q_values):
            if q_values.size(1) > valid_actions:
        ...
        if self.dqn_net.training:
            ...
            if self.epsilon > np.random.random():
                ...
                q_values = mask_invalid(self.dqn_net(state))
    ...
    def updateModel(self):
        ...
        if hasattr(self.memory, "sample_batch"):
            (state, action, reward,
             next_state, done,
             mask, next_mask) = self.memory.sample_batch()
            state = torch.FloatTensor(state).to(self.device)
            ...
            next_mask = torch.BoolTensor(next_mask).to(self.device)

            q_eval = self.dqn_net(state)
            q_eval[~mask] = -1e9
            q_eval = q_eval.gather(1, action)

            q_next = self.dqn_target_net(next_state).detach()
            q_next[~next_mask] = -1e9
            q_target = reward + self.discount_factor * (1 - done) * q_next.max(1, keepdim=True)[0]
        ...
        with open(csv_path, "w", newline="", encoding="utf-8") as f:
            ...
            for idx in range(max_len):
                writer.writerow([
                    ...
                    pick(self.episode_mean_rewards, idx),
    ...
    def _save_convergence_figure(self, base_dir, time_str):
        ...
        axes[0].plot(
            range(1, len(self.mean_losses) + 1),
            self.mean_losses,
            "-o",
            linewidth=1,
            markersize=2,
        ...
        reward_curve = self.episode_mean_rewards if self.episode_mean_rewards else self.mean_rewards
        reward_xlabel = "Episode" if self.episode_mean_rewards else f"Target-network update window ({self.target_update} gradient steps)"
        axes[1].plot(
            range(1, len(reward_curve) + 1),
            reward_curve,
            "-o",
            linewidth=1,
            markersize=2,
        )
        axes[1].set_xlabel(reward_xlabel)
        axes[1].set_ylabel("Mean Reward")
        axes[1].grid(True, alpha=0.3)

        axes[2].plot(
            range(1, len(self.epsilons) + 1),
            self.epsilons,
            linewidth=1,
        )
    ...
    def trainSave(self, more_text="", mean_makespan=[], mean_cost=[], 
        ...
        base_dir = "logs"  # 与 A3C 对齐：保存到 logs 根目录
        ...
        # A3C 风格：按 episode 平均奖励作图；若没有，退回 target_update 窗口均值
        if self.episode_mean_rewards:
            plt.plot(range(1, len(self.episode_mean_rewards) + 1), self.episode_mean_rewards, '-o', linewidth=1, markersize=2)
            plt.xlabel("Episode")
            plt.ylabel("Average Reward (per Episode)")
            plt.savefig(f"{base_dir}/{time_str}_reward.png", facecolor='w')
            plt.show()
            plt.clf()
        else:
            plt.plot(self.mean_rewards, '-o', linewidth=1, markersize=2);
        ...
        if mean_cost:
            ...
            plt.xlabel("Episode");
            plt.ylabel("Cost");
            plt.savefig(f"{base_dir}/{time_str}_cost.png", facecolor='w');
            plt.show();
            plt.clf();
        ...
        with open(f"{base_dir}/{time_str}_train-atten.txt",'w') as f:
        ...
        try:
            model_path = f"logs/{time_str}_dqn_agent.pkl"
