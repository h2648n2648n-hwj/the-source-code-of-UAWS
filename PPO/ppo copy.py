...
class PPOScheduler:
    ...
    def costReward(self, state, action):
        ...
        min_cost = float(torch.min(cost).item()) if torch.is_tensor(cost) else min(cost)
        max_cost = float(torch.max(cost).item()) if torch.is_tensor(cost) else max(cost)

        if max_cost == min_cost:
            cost_r = 1.0
        else:
            c_action = float(cost[action].item() if torch.is_tensor(cost[action]) else cost[action])
            cost_r = 1.0 - (c_action - min_cost) / (max_cost - min_cost)
        return float(cost_r)
    ...
    def timeReward(self, state, action):
        ...
        if t_action <= deadline:
            ...
            if abs(t_max - deadline) > 1e-12:
                ...
                time_r = -1.0
            # time_r *= self.time_penalty_factor
        return float(time_r)
    ...
    def schedule1(self, last_part, task, vm_list, ready_queue, remained_task, all_task_num, now_time, done, reward_env=None):
        # 严格以“当前时刻”的状态为基准
        state_now = self.createState(task, vm_list, ready_queue, remained_task, all_task_num, now_time).to(self.device)
        ...
        if not last_part:
            ...
            return vm_list[int(provisional_action.item())], True
        ...
        return vm_list[int(action.item())], True
