...
# 导入必要的模块
# from dqn.dqn import DQNScheduler
from dqna.dqn import DQNScheduler
from env import IaaS, Workload
from env.workflow import Workflow
from rdws import runEnv, setRandSeed

def load_trained_dqn_model(model_path):
    """
    加载训练好的DQN模型
    ...
    try:
        ...
        # 设置为评估模式
        if hasattr(agent, 'dqn_net') and agent.dqn_net:
            agent.dqn_net.eval()
        if hasattr(agent, 'dqn_target_net') and agent.dqn_target_net:
            agent.dqn_target_net.eval()
            
        print(f"DQN模型加载成功: {model_path}")
        print(f"模型配置:")
        print(f"  - 动作空间大小: {agent.action_num}")
        print(f"  - 状态维度: {agent.state_dim}")
        print(f"  - 奖励函数编号: {agent.reward_num}")
        print(f"  - Alpha参数: {agent.alpha}")
...
def test_dqn_performance(agent, test_episodes, workflows_per_episode, 
    ...
    for episode in range(1, test_episodes + 1):
        ...
        def scheduler_wrapper(*args):
            return agent.schedule1(*args)  # 只返回虚拟机，忽略布尔值
...
def main():
    ...
    # 配置参数
    # model_path = "/root/common-dir/RDWS/logs/1_0.5"  # 训练好的DQN模型路径
    # model_path = "/root/common-dir/RDWS/logs/a_dqn_agent.pkl"  # 训练好的DQN模型路径
    # model_path = "/root/common-dir/RDWS/logs/2025-11-16 22:21 _dqn_agent.pkl"  # 训练好的DQN模型路径
      # 训练好的DQN模型路径
    test_episodes = 10  # 测试轮数    docker run --gpus all -it <image> bash    docker run --gpus all -it <image> bash
    workflows_per_episode = 10  # 每轮工作流数
    test_wf_path = "workflows/SyntheticWorkflows/test_all_300"  # 测试工作流路径
    ...
    arrival_rate = 0.1/60  #0.1/60  # 工作流到达率
    random_seed = 42  # 随机种子
    ...
    # 1. 加载训练好的DQN模型
    agent = load_trained_dqn_model(model_path)
    if agent is None:
        print("DQN模型加载失败，退出测试")
        return
